...
var pool = module.exports = function pool(options, authorizeFn) {
    ...
    function StartStratumServer(finishedCallback) {
        ...
        _this.stratumServer.on('started', function () {
            ...
            client.on('client.authorized', function (client) {
                const m = /;?d=(\d*(\.\d+)?)/.exec(client.workerPass);
                if (m) {
                    const staticDiff = parseFloat(m[1]);
                    client.enqueueNextDifficulty(staticDiff);
                } else if (typeof (_this.varDiff[client.socket.localPort]) !== 'undefined') {
